...
// 先从aptr和bptr中取出internal_key，再比较两个internal_key
...
// target是一个internal_key而不是user_key，根据internal_key构造出一个memtable_key
...
bool MemTable::Get(const LookupKey& key, std::string* value, Status* s) {
  ...
  if (iter.Valid()) {
    ...
    const char* entry = iter.key();// 得到memtable_key
